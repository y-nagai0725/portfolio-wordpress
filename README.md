...
アニメーション処理はGSAPを使用しており、アニメーション完了時のコールバック処理を `onComplete` を使用して実装しています。SVGの `path` 要素へのクラス付与によってアニメーションのタイミングをずらしたり、描画後にフェードアウトするといった処理もコールバックによって個別に実装しています。
